                    ...
                    if (canLoadMore) {
                        EdgeButton(
                            onClick = {
                                ...
                                val nextItemsCount = minOf(5, transactions.size - currentCount)
                                if (nextItemsCount > 0) {
                                        visibleTransactions =
                                            transactions.take(currentCount + nextItemsCount)
                                        canLoadMore = transactions.size > visibleTransactions.size
                                }
                            },
...
@Composable
private fun ScrollViewContent(listState: TransformingLazyColumnState, contentPadding: PaddingValues, transactions: List<Transaction>) {
